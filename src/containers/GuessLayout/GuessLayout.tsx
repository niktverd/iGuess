        ...
        return (
            <div className="custom-tooltip" style={{...contentStyle, padding: 15}}>
                ...
                {payload.map((pl: Record<string, string>, index: number)=> {
                    ...
                    const newLabel = labelParts.map((lp: string) => {
                    ...
                    return <p key={pl.name} style={{color: pl.color}} className="label">{`${newLabel.join(' ')} : ${payload[index].value}`}</p>
                })}
            </div>
        );
    }
...
export const GuessLayout = (_props: GuessLayoutProps) => {
    ...
    const handleParameterAxisSelection = (paramName: string, step: number) => () => {
        let value = typeof axisByParameter[paramName] === 'number' ? axisByParameter[paramName] + step : 0;
        if (value < 0) {
            value = 0;
        } 
                ...
                {(section === Section.Overview && flatData) ?
                    <div>
                        ...
                        <div className={styles['chars-layout']}>
                            <div>{sourceData.plans.map((p) => {
                                return <button key={p.id} onClick={handlePlanSelection(p.id)}>{p.name}</button>
                            })}</div>
                            <div>
                                <LineChart
                                    width={800}
                                ...
                                >
                                    <XAxis dataKey="month" />
                                    <Tooltip contentStyle={{ backgroundColor: "black" }} content={<CustomTooltip />}/>
                                    ...
                                    {uniq(dataKeys).map((dk) => {
                                        ...
                                        return <Line key={dk} type="monotone" dataKey={dk} stroke={randomHex({ min: 150 })} yAxisId={axisId || 0} />
                                    })}
                                </LineChart>
                            </div>
                            <div>
